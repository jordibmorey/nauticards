...
const regionsById = regionsList ? indexById(regionsList) : null;
    ...
    /* =========================
   SEO DINÁMICO EMPRESA
========================= */
{
  const seoCompanyName = (company?.name || "").trim();

  // Intenta sacar ubicación (si existe en tu app)
  const seoPortObj =
    (typeof portsById !== "undefined" && portsById?.get?.(String(company?.port_id))) ||
    (typeof portsById !== "undefined" && portsById?.[String(company?.port_id)]) ||
    null;
  ...
  // TITLE
  const seoTitleParts = [
    seoCompanyName || "Empresa",
    seoPortName || null,
    "NautiCards"
  ].filter(Boolean);

  document.title = seoTitleParts.join(" | ");

  // DESCRIPTION (corta y clara)
  const seoLocation = seoPortName ? ` en ${seoPortName}` : "";
  const seoDesc =
    `${seoCompanyName || "Esta empresa"} es una empresa náutica${seoLocation}. ` +
    `Consulta servicios, contacto y ubicación en NautiCards.`;

  // meta description: buscar o crear
  let seoMetaDesc = document.querySelector('meta[name="description"]');
  if (!seoMetaDesc) {
    seoMetaDesc = document.createElement("meta");
    seoMetaDesc.setAttribute("name", "description");
    document.head.appendChild(seoMetaDesc);
  }
  seoMetaDesc.setAttribute("content", seoDesc);
}

/* =========================
   OPEN GRAPH / TWITTER - EMPRESA
========================= */
{
  ...
  // company.logo en tu JSON (a veces viene con //... sin protocolo)
  let ogImage = (company?.logo || "").trim();
  if (ogImage.startsWith("//")) ogImage = `https:${ogImage}`;

  // fallback seguro (este archivo existe en tu ZIP)
